...
$_SESSION['login'] = NULL ;
								                // On vérifie que les trois input 'login','pass' et 'pass' deux sont remplis et si ils sont remplis on associe une variable au mot rentré dans l'input
                ...
                if (isset($_POST['pass2'])) 
                {
                       $prefixe="m0td3p4ss3dEbut";
                        $suffixe="mOtdEpAssEf1N";
                                $pass2 = md5($prefixe.$_POST['pass2'].$suffixe);
                }
                if (isset($_POST['mail'])) 
                {
                                $mail=$_POST['mail'];
...
// On vérifie que l'utilisateur a envoyé le formulaire
    ...
    $query = mysqli_query($conn2,"SELECT US_pseudo FROM User WHERE US_pseudo = '$login'");
        ...
        if(isset($_POST['login']) && !empty($_POST['login']) && isset($_POST['pass']) && !empty($_POST['pass']) 
           && isset($_POST['pass2']) && !empty($_POST['pass2']) && isset($_POST['mail']) && !empty($_POST['mail']))
        {
        ...
        echo 'Veuillez remplir les champs obligatoires s\'il vous plait';
            ...
            if(mysqli_num_rows($query) == 0)
            {
                ...
                {
                    if(preg_match("/(([a-z][0-9])|([0-9][a-z])|[A-Z][0-9]|([0-9][A-Z]))/",$_POST['pass']))
                    {
                        if(strlen($_POST['pass'])>6)
                        {
                            ...
                            {   
                                ...
                                {                                
                                    ...
                                    echo "<p style='text-align:center;'> Votre compte a été créé avec succés !</p>";
                                }else 
                                {
                        ...
                        {
                            echo "Votre mot de passe doit avoir plus de 6 caractères ";
                        }
                    }else
                        { 
                        echo "Votre mot de passe doit contenir au moins une lettre majuscule et un chiffre." ;
                    }
                }else{
                    echo "<p style='text-align:center;'>Un compte associé à ce mail existe déjà !</p>";
                }
            }else{
                echo "<p style='text-align:center;'>  Le pseudo existe déjà ! </p>";
            }
    }
?>
<br />
